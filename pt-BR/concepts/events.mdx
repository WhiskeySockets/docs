---
title: "Eventos"
description: "Eventos tipados para mensagens, conexão, grupos, contatos e mais."
---
...
sock.ev.on('messages.update', async (updates) => {
  for (const { key, update } of updates) {
    ...
    if (update.pollUpdates) {
      // decifra votos da enquete — requer `getMessage` configurado no SocketConfig
  ...
  sock.ev.process(async (events) => {
    ...
    if (events['messages.update']) {
      for (const { key, update } of events['messages.update']) {
        if (update.pollUpdates) {
          // `getMessage` precisa ser passado em SocketConfig (veja a seção `getMessage` em socket-config)
          const pollCreation = await getMessage(key)
          if (pollCreation) {
            console.log(
              'Poll results:',
