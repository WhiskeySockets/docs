---
title: "Connect with a pairing code"
description: "Link Baileys to WhatsApp Web without scanning a QR code using a 8-digit pairing code entered on your phone. Covers phone number format and setup flow."
---
...
<Steps>
  <Step title="Create the socket with printQRInTerminal disabled">
    You must set `printQRInTerminal` to `false` when using pairing codes. If it is `true`, Baileys will attempt to display a QR code and the pairing code flow will not work correctly.
